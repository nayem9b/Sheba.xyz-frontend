...
const AllUsers = () => {
  ...
  // useEffect(() => {
  //   fetch(`https://sheba-backend.vercel.app/api/v1/users/db/${fetchFromDB}`)
  //     .then((res) => res.json())
  //     .then((data) => {
  ...
  const deleteHandler = async (id: string) => {
    ...
    fetch(`https://sheba-backend.vercel.app/api/v1/users/db/${id}`)
      .then((res) => res.json())
      .then((data) => {
        console.log(data?.data);
        setDeleteUserId(data?.data?.userId);
    ...
    try {
      fetch(`https://sheba-backend.vercel.app/api/v1/users/${deleteUserId}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
